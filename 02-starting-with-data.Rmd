...
Inside the `download.file` command, the first entry is a character string with
the source URL ("<https://ndownloader.figshare.com/files/2292169>"). This source
URL downloads a CSV file from figshare. The text after the comma
("data\_raw/portal\_data\_joined.csv") is the destination of the file on your
local machine. You'll need to have a folder on your machine called "data\_raw"
where you'll download the file. So this command downloads a file from Figshare,
names it "portal\_data\_joined.csv" and adds it to a preexisting folder named
"data\_raw".
...
need to re-install packages every time we run the script. Then, to load the
package type:
...
looks through the first 1000 rows of each column and guesses its data type. For
example, in this dataset, `read_csv()` reads `weight` as `col_double` (a numeric
data type), and `species` as `col_character`. You have the option to specify the
data type for a column manually by using the `col_types` argument in `read_csv`.
...
used as a field delineator. If you want to read in this type of files in R, you
can use the `read_csv2()` function. There is also the `read_tsv()` for tab
separated data files and `read_delim()` for less common formats. Check out the
help for `read_csv()` by typing `?read_csv` to learn more.

<aside>

Knowing the parameters of your data files will ease up the import step. It is
good to develop the habits of looking at and recording parameters of your csv
files such as the character encoding, control characters used for line ending,
date format (if the date is not splitted into three variables), and the presence
of unexpected [newlines](https://en.wikipedia.org/wiki/Newline).
...
`surveys`. By default, this will show show you as many rows and columns of the
...
lines of the data set. It is called (you might have guessed it) `tail()`.
...
[here](https://tibble.tidyverse.org/)). Data frames are the *de facto* data
...
![](./img/data-frame.svg) We can see this also when inspecting the
<b>str</b>ucture of a data frame with the function `str()`:
...
of the class `character`. Arguably, these columns contain categorical data, that
is, they can only take on a limited number of values.
...
again. This produces a table with the counts for each factor level:

```{r, purl=FALSE}
summary(surveys$sex)
```

By default, R always sorts levels in alphabetical order. For instance, if you
have a factor with 2 levels:
...
`f` comes before `m`, even though the first element in this vector is `"male"`).
You can see this by using the function `levels()` and you can find the number of
...
`"male"` is more descriptive than `1`, `2`. Which one is "male"? You wouldn't be
able to tell just from the integer data. Factors, on the other hand, have this
...
Let's look at the number of males and females captured over the course of the
experiment:
...
Howver, as we saw when we used `summary(surveys$sex)`, there are about 1700
...
to work with a copy of the `sex` column, so we're not modifying the working copy
of the data frame:
...
-   Now that we have renamed the factor level to "undetermined", can you
    recreate the barplot such that "undetermined" is last (after "male")?
...
packages used in most data analyses) get loaded. **`lubridate`** however does
...
Now we apply this function to the surveys dataset. Create a character vector
from the `year`, `month`, and `day` columns of `surveys` using `paste()`:
...
but first we add the resulting `Date` vector to the `surveys` data frame as a
