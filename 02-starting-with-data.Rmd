...
name: `surveys`. By default, this will show show you as many rows and columns of
...
ere is also the `read_tsv()` for tab separated data files and `read_delim()` 
...
habits of looking at and record some parameters of your csv files. For instance, 
the character encoding, control characters used for line ending, date format
(if the date is not splitted into three variables), and the presence of 
unexpected [newlines](https://en.wikipedia.org/wiki/Newline) are important
characteristics of your data files.
Knowing those parameters will ease up the import step of your data in R.
</aside>
