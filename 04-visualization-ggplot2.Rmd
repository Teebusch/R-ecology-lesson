...
-   The `+` sign used to add layers must be placed at the end of each line
    containing a layer. If, instead, the `+` sign is added in the line before
    the other layer, **`ggplot2`** will not add the new layer and will return an
    error message.
...
>
> -   Add color to the data points on your boxplot according to the plot from
...
Let's calculate number of counts per year for each genus. First we need to group
...
together. We need to tell ggplot to draw a line for each genus by modifying the
aesthetic function to include `group = genus`:
...
different functions in a sequence and create a coherent workflow. We can also
use the pipe operator to pass the `data` argument to the `ggplot()` function.
The hard part is to remember that to build your ggplot, you need to use `+` and
not `%>%`.
...
use it to make a time series plot for each species:
...
**Note:** `ggplot2` before version 3.0.0 used formulas to specify how plots are
faceted. If you encounter `facet_grid`/`wrap(...)` code containing `~`, please
read <https://ggplot2.tidyverse.org/news/#tidy-evaluation>.
...
Now, let's change names of axes to something more informative than 'year' and
'n' and add a title to the figure:
...
labels. We can also modify the facet label text (`strip.text`) to italicize the
genus names:
...
> beautiful graph of your own. Use the RStudio [**`ggplot2`** cheat
...
You can also use parentheses `()` to create more complex layouts. There are many
useful examples on the [patchwork
